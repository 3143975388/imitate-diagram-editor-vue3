  ...
  <div class="app-header">
    ...
    >
      ...
      <t-dropdown-menu>
        ...
        <t-dropdown-item @click="saveDraw" divider="true">
          <a>保存</a>
        </t-dropdown-item>
...
// @ts-ignore
  import { addScadaPaperImage,getScadaPaperImageList,addScadaDirectory,getScadaDirectory ,uploadFile,updateScadaPaperImage} from '@/api/photo/photo.ts';
...
const saveDraw = () => {
  // 正确上传meta2d生成的PNG图像
  const blob = meta2d.toPng();
  const fos = base64ToBlob(blob);

  // 创建FormData对象
  const formData = new FormData();
  formData.append('file',fos); // 添加文件，指定文件名
  ...
  uploadFile(formData).then((res:any) => {
    if (res.code === 200) {
      ...
      if (photoDatas) {
        ...
        // 更新图纸图片
        updateScadaPaperImage(photoData).then(() => {
 ...
 * @param {string} base64String - Base64 编码的字符串（可包含 data URI 前缀）
 * @param {string} [contentType='application/octet-stream'] - 数据类型（MIME 类型）
 * @returns {Blob} 转换后的 Blob 对象
 */
function base64ToBlob(base64String: string, contentType = 'application/octet-stream') {
  // 提取 Base64 数据部分（去除 data URI 前缀，如 "data:image/png;base64,"）
  const base64Data = base64String.includes(',') 
    ? base64String.split(',')[1] 
    : base64String;
  
  // 自动解析 MIME 类型（如果 base64String 包含 data URI 前缀）
